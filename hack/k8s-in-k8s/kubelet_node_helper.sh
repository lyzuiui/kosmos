...
JOIN_TOKEN=$3
JOIN_CA_HASH=$4
NODE_LOCAL_DNS_ADDRESS=169.254.20.10
...
function join() {
    ...
    sed  -e "/__DNS_ADDRESS__/i - $NODE_LOCAL_DNS_ADDRESS" -e "s|__DNS_ADDRESS__|$DNS_ADDRESS|g" -e "w ${PATH_KUBELET_CONF}/${KUBELET_CONFIG_NAME}" "$PATH_FILE_TMP"/"$KUBELET_CONFIG_NAME"
    if [ $? -ne 0 ]; then
        exit 1
    fi
